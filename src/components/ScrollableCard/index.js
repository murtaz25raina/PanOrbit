  ...
  return (
    <div className="srollable-card">
      ...
      <div className="contact-list">
        {props.contacts && props.contacts.hasOwnProperty('users') && props.contacts.users.length > 0 ? (
          props.contacts.users.map((contact) => {
            return (
                <Link key={contact.id} className="link-card" to={`home/profile/${contact.id}`} onClick={()=>loginHandler(contact.id)}>
